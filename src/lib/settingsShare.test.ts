...
describe('settingsShare', () => {
  it('round-trips an empty inventory', () => {
    const decoded = decodeSettings(encodeSettings({ ownedThreadCodes: [], sizeUnit: 'cm', printMode: 'auto', printColorMode: 'color' }))
    expect(decoded).toEqual({ ownedThreadCodes: [], sizeUnit: 'cm', printMode: 'auto', printColorMode: 'color' })
  })

  it('round-trips a handful of owned codes and the size unit', () => {
    const codes = ['310', 'B5200', 'E3852', 'S310']
    const decoded = decodeSettings(encodeSettings({ ownedThreadCodes: codes, sizeUnit: 'in', printMode: 'auto', printColorMode: 'color' }))
  ...
  it('round-trips every known thread owned', () => {
    ...
    const decoded = decodeSettings(encodeSettings({ ownedThreadCodes: codes, sizeUnit: 'cm', printMode: 'auto', printColorMode: 'color' }))
    expect(decoded?.ownedThreadCodes.sort()).toEqual([...codes].sort())
  })

  it('ignores unknown codes when encoding (they cannot be represented in the bitset)', () => {
    const decoded = decodeSettings(encodeSettings({ ownedThreadCodes: ['not-a-real-code'], sizeUnit: 'cm', printMode: 'auto', printColorMode: 'color' }))
  ...
  it('produces a token shorter than a naive comma-joined list, for a realistic owned count', () => {
    ...
    const token = encodeSettings({ ownedThreadCodes: codes, sizeUnit: 'cm', printMode: 'auto', printColorMode: 'color' })
